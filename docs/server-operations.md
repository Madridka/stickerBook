...
В `STICKER_BOOK_LOG_FILE` можно указать путь к NDJSON-файлу или существующему каталогу.
Для каталога сервер использует имя `sticker-book.ndjson`. Уровень задаётся через
