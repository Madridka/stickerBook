...
This directory powers the registered `ucl-26-27` journal at `/album/ucl-26-27`. The album contains five navigation/editorial pages and one two-page, 20-card spread for each of 28 currently listed clubs. Final card assets live in `public/ucl-26-27/cards`; album page art lives in `assets/game/ucl-26-27/main/album`.

## Snapshot

- Snapshot date: `2026-08-06`.
- Status: `provisional`; 28 clubs are represented and eight remaining league-phase places are intentionally omitted until qualification is settled.
...
No official tournament logo or downloaded tournament branding is stored here. The 22 provisional additions use reproducible illustrated WebP placeholders generated by `scripts/generate-ucl-provisional-clubs.py`; replacing an image later does not require catalog or geometry changes. Adding another club requires a manifest entry and a `cards.json`; geometry and contents derive their club lists from the manifest.
