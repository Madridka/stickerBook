...
const collection = vi.hoisted(() => ({
  getAlbumProgress: vi.fn((albumId: string) => ({
    albumId,
    totalCards: albumId === 'ucl-26-27' ? 851 : 0,
    collectedCards: albumId === 'ucl-26-27' ? 43 : 0,
    placedCards: 0,
    duplicateCards: 0,
    completionPercent: albumId === 'ucl-26-27' ? 5 : 0,
  })),
}))
...
describe('AlbumLibraryView', () => {
  ...
  it('switches real tabs and shows collection completion for UCL', async () => {
    ...
    expect(uclAlbum?.text()).toContain('Заполнено: 5%')
    expect(uclAlbum?.text()).toContain('Собрано 43 из 851')
    expect(uclAlbum?.find('[style="width: 5%;"]').exists()).toBe(true)
